...
<div class="form_box">
	...
	<p><strong><%out.println(request.getAttribute("titleMsg")); %></strong></p>
	<div>
		<label>标题</label>
		<input type="text" name="inform.title" value="${title}"/>
	</div>
	<p><strong><%out.println(request.getAttribute("contentMsg")); %></strong></p>
	<div>
		<label>内容</label>
		<textarea name="inform.content" cols="80" rows="10">${content}</textarea>
	</div>
	<p>以下为敏感信息，如不方便可不填写</p>
	<div>
		<div>
		<label>您的电话联系方式</label>
		<input type="text" name="inform.phone" value="${phone}"/>
	</div>
	<div>
		<div>
		<label>您的姓名</label>
		<input type="text" name="inform.name" value="${name}"/>
	</div>
	<div>
		<label>&nbsp;</label>
